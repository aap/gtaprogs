...
mouseMotion(GLFWwindow *w, double x, double y)
{
	 ...
	 * Unfortunately, after glfwSetCursorPos is done, there can be old
...
dispatchInput(Pad &pad)
{
	...
	if(pad.id >= 0){
		...
		cam->orbit(-pad.axes[2]/30.0f, pad.axes[3]/30.f);

		float lr = pad.axes[0];
		float ud = pad.axes[1];
		padDir = Vec3(0.0f, 1.0f, 0.0f)*ud +
			 Vec3(1.0f, 0.0f, 0.0f)*lr;
		if(padDir.norm() > 0.0f){
			padDir /= padDir.norm();
			float speed = ud*ud + lr*lr;
			if(speed > 1.0f) speed = 1.0f;
			padDir *= speed;
		}
	}

	float kbSpeed = kbDir.norm();
	float padSpeed = padDir.norm();
	float speed = kbSpeed > padSpeed ? kbSpeed : padSpeed;
	Vec3 dir = kbDir + padDir;
	float n = dir.norm();
	if(n > 0.1){
		dir /= dir.norm();
		float a = cam->getHeading()/2.0f;
		Quat q(cos(a), 0, 0, -sin(a));
		dir = Vec3(q.K() * Quat(dir) * q);
		playerped->handleInput(0, dir*speed);
	}else
		playerped->handleInput(0, Vec3(0.0f, 0.0f, 0.0f));
